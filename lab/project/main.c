...
struct avl_music *createNewMusic(char *judul, char *penyanyi, char *tahun) {
    struct avl_music *newMusic = (struct avl_music *)malloc(sizeof(struct avl_music));
...
struct avl_music *insert(char *judul, char *penyanyi, char *tahun, struct avl_music *root) {
...
struct avl_data_table table[1000]; // untuk bantuan pilih delete music berdasarkan angka

void printInOrder(struct avl_music *root) {
    if (root != NULL) {
        printInOrder(root->left);
        printf("| %-3d | %-30s | %-30s | %-15s |\n", avl_count_music + 1, root->dataMusic.judul, root->dataMusic.penyanyi, root->dataMusic.tahun);
...
// delete music
int count_playlist = 0; // dipake untuk menghitung berapa banyak playlist yang udah dibuat
char copy_str_judul_musik[30];  // untuk menyimpan judul musik mana yang akan terhapus
// deleteNode memerlukan nilai dari input pilih musik nomor berapa, dan juga max musik yang ada pada playlist tersebut
void adminDeleteMusic() {
    ...
    printf("%s", "-------------------------------------------------------------------------------------------\n");
    printf("| %-3s | %-30s | %-30s | %-15s |\n", "No", "Judul Lagu", "Penyanyi", "Tahun Rilis");
    printf("%s", "-------------------------------------------------------------------------------------------\n");
    printInOrder(root);
    printf("%s", "-------------------------------------------------------------------------------------------\n\n");
    ...
    do {
        ...
        scanf("%d", &input);
        getchar();
    } while (input <= 0 || input > avl_count_music); // akan keluar looping kalau nilai yg dipilih di antara 1 sd. maks musik yg ada
    ...
    for(int i = 0; i < count_playlist; i++){    // akan dilakukan perintah ini kalau count_playlist minimal = 1
        if(playlists[i]->head != NULL){
            ...
            if(strcmp(curr->dataMusic.judul, table[input-1].judul) == 0){
                if (playlists[i]->head == playlists[i]->tail) { // kalo ternyata head itu sama dengan tail, artinya cuma ada 1 musik, maka head dan tail nya jadii NULL
                ...
                strcpy(copy_str_judul_musik, curr->dataMusic.judul);    // copy dulu judul musik yang mau dihapus
                ...
                playlists[i]->tail->next = playlists[i]->head;
                playlists[i]->head->prev = playlists[i]->tail;
                strcpy(copy_str_judul_musik, curr_t->dataMusic.judul);    // copy dulu judul yg mau dihapus
                ...
                while(curr != playlists[i]->tail){
                    if(strcmp(curr->dataMusic.judul, table[input-1].judul) == 0){
                        // kalo user delete musik yang berada di tengah, maka harus traverse dulu dari head sampai nomor input yang dipilih
                        curr->prev->next = curr->next;
                        curr->next->prev = curr->prev;
                        strcpy(copy_str_judul_musik, curr->dataMusic.judul);    // copy dulu judul yg mau dihapus
                        free(curr); // baru hapus node judul musik tersebut
                        printf("\n- %s berhasil dihapus pada playlist [%s]\n", table[input-1].judul, playlists[i]->namaPlaylist);
                    }
                    curr = curr->next;
...
void savePlaylisttoFile(){
    ...
    for(int i = 0; i < count_playlist; i++){
        ...
        if(curr != NULL){
            while(curr != playlists[i]->tail){
                fprintf(fps, "%s-%s-%s#.\n", curr->dataMusic.judul, curr->dataMusic.penyanyi, curr->dataMusic.tahun);
                curr = curr->next;
            }
...
struct node *createNewNode(char tempjudul[], char temppenyanyi[], char temptahun[]) {
...
int userChoosePlaylist() {
    ...
    if (count_playlist == 0) {
        ...
        do{
            ...
            scanf("%d", &playlistNum);
            getchar();
        }while (playlistNum  <= 0 || playlistNum > count_playlist); // akan keluar looping kalau nilai yg dipilih di antara 1 sd. maks musik yg ada
...
int user_flag_err_delete_music = 0; // variable untuk menandai harus print message yg mana ke layar
void userAddtoPlaylist() {
    ...
    if (playlistNum == -1) {    // kalo 0-1 = -1, artinya ga ada playlist
        ...
        avl_count_music = 0;    // variable awal untuk menyimpan informasi berapa banyak musik yang ada di avl
        ...
        // ketika avlShowMusic, di situ lah akan traverse secara in order, dan di situ juga dilakukan string copy ke array table
        struct node *newNode = createNewNode(table[input-1].judul, table[input-1].penyanyi, table[input-1].tahun);

        // kalo awal-awal playlist masih kosong, maka head dan tail di-update nge-pointing ke new node
...
int userDisplayMusicPlaylist() {
    ...
    user_flag_err_delete_music = 0; //flag penanda print message tertentu jadiin 0

    int playlistNum = userChoosePlaylist();
    cls;

    int linkedList_count_music = 0; // variable untuk menghitung seberapa banyak musik yang ada pada 1 playlist tertentu
    ...
    if (playlistNum == -1) {
        ...
        struct node *curr = playlists[playlistNum]->head;   // awal-awal curr nge-pointing ke head pada playlist yang dipilih

        if (playlists[playlistNum]->head == NULL) { // kalo head nya NULL, berarti ga ada musik sama sekali
            printf("| %-3d | %-30s | %-30s | %-15s |\n", 0, "NULL", "NULL", "NULL");
            ...
            while (curr != playlists[playlistNum]->tail) {
                linkedList_count_music++;   // variable diiterasikan untuk merepresentasikan banyaknya musik yang ada
                printf("| %-3d | %-30s | %-30s | %-15s |\n", linkedList_count_music, curr->dataMusic.judul, curr->dataMusic.penyanyi, curr->dataMusic.tahun);
                curr = curr->next;  // traverse si curr
            }
            linkedList_count_music++;   // +1 karena di bawah akan dihitung dan di-print untuk node tail nya
            printf("| %-3d | %-30s | %-30s | %-15s |\n", linkedList_count_music, curr->dataMusic.judul, curr->dataMusic.penyanyi, curr->dataMusic.tahun);
        }
        // print divider penutup
        printf("%s", "-------------------------------------------------------------------------------------------\n");
        if(user_flag_err_delete_music == 1){    // kalo flag-nya aktif alias = 1, maka print message berikut
            printf("Tambahkan musik ke playlist terlebih dahulu di [menu 2]\n\n");
        }
    }
    // me-return nilai dari berapa banyaknya musik yang ada pada suatu playlist yang dipilih
...
void deleteNode(int input, int linkedList_count_music) {
    ...
    playlistNum = playlistNum-1;    // walau nilai dari playlistNum itu udah di-return -1, tapi sayangnya itu ga mengubah nilai playlistNum asli yang mana di-passing ke function deleteNode. Sehingga perlu dilakukan -1

    struct node *curr = playlists[playlistNum]->head;   // tempatin curr pertama kali di head
    
    if (input == 1) {   // kalo user pilih 1, berarti kita delete node pertama alias si head
        if (playlists[playlistNum]->head == playlists[playlistNum]->tail) { // kalo ternyata head itu sama dengan tail, artinya cuma ada 1 musik, maka head dan tail nya jadii NULL
    ...
    else if (input == linkedList_count_music) { // kalo input itu sama dengan maks. musik yang ada, artinya user ingin delete tail
...
void userDeleteMusicPlaylist() {
    ...
    if (linkedList_count_music > 0) {
        ...
        do {
            ...
            scanf("%d", &input);
            getchar();
        } while (input <= 0 || input > linkedList_count_music); // akan keluar looping kalau nilai yg dipilih di antara 1 sd. maks musik yg ada
...
void fileAddPlaylist(char tempJudul[], char tempPenyanyi[], char tempTahun[], int count_playlist){
...
void readFile() {
    ...
    char tempNamaPlaylist[30], tempJudul[30], tempPenyanyi[30], tempTahun[5], tempStatus;
    while (!feof(fp)) {
        fscanf(fp, " %[^#]#%[^#]#%s\n", tempJudul, tempPenyanyi, tempTahun);
    ...
    if(!feof(fpr)){
        ...
        while (!feof(fpr)){    
            fscanf(fpr, " %[^-]-%[^-]-%[^#]#%c\n", tempJudul, tempPenyanyi, tempTahun, &tempStatus);
            
            fileAddPlaylist(tempJudul, tempPenyanyi, tempTahun, count_playlist);
          
            // ditandai tempStatus = "*" artinya sudah sampai tail di satu playlist, untuk playlist selanjutnya akan lanjut dibaca sampai bertemu"*" lagi 
            if (tempStatus == '*'){
                count_playlist++;
                fscanf(fp, "\n");
